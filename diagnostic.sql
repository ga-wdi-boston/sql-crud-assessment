...
-- See `data/licensees.csv` for example data.  Don't forget that all tables should have an id column as a unique identifier for a row.
...
SELECT * FROM licenses WHERE eye_color = 'Hazel' and hair_color in ('Brown', 'Black');
...
DELETE * FROM licenses WHERE (surname = 'Rich' and given_name = 'Dylan') and (surname = 'Myers' and given_name = 'Teresita');
