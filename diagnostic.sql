...
INSERT INTO licensees(surname, given_name, gender, height, weight, born_on, eye_color, hair_color) VALUES
...
SELECT * FROM licensees WHERE eye_color='Hazel' AND hair_color IN ('Brown', 'Black');

-- Question 4. Make each of Marilynn Escobar and Chris Whaley one inch shorter. Look them up by their names.

UPDATE licensees
SET height = height - 1
WHERE surname IN ('Escobar', 'Whaley') AND given_name IN ('Marilynn', 'Chris');

-- code to check before/after update
SELECT * FROM licensees WHERE surname IN ('Escobar', 'Whaley') AND given_name IN ('Marilynn', 'Chris');
-- Question 5. Remove Dylan Rich and Teresita Myers from the table of licensees.

-- your code here
DELETE FROM licensees
WHERE surname IN ('Rich', 'Myers') AND given_name IN ('Dylan', 'Teresita');

-- code to check before/after update
SELECT * FROM licensees WHERE surname IN ('Rich', 'Myers') AND given_name IN ('Dylan', 'Teresita');
