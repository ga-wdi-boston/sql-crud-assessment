...
SELECT licensees, COUNT(*)
FROM licensees
WHERE eye_color LIKE 'Hazel' AND WHERE hair_color LIKE 'Brown' OR 'Black' -- Authors whose name starts with an "E"
ORDER BY COUNT (*);

-- Question 4. Make each of Marilynn Escobar and Chris Whaley one inch shorter. Look them up by their names.
UPDATE patients
SET weight = weight * 1.05;


UPDATE licensees
SET height = height + 1
WHERE surname LIKE 'Escobar' AND 'Whaley'


-- Question 5. Remove Dylan Rich and Teresita Myers from the table of licensees.
REMOVE licensees
WHERE surname LIKE 'Rich' and 'Myers'
