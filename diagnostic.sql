...
SELECT * FROM licensees WHERE eye_color = 'Hazel' OR hair_color = 'Black';

-- Question 4. Make each of Marilynn Escobar and Chris Whaley one inch shorter. Look them up by their names.

UPDATE licensees SET height = height - 1 WHERE given_name || ' ' || surname in ('Marilynn Escobar', 'Chris Whaley');


-- Question 5. Remove Dylan Rich and Teresita Myers from the table of licensees.

DELETE FROM licensees WHERE given_name || ' ' || surname in ('Dylan Rich', 'Teresita Myers');
