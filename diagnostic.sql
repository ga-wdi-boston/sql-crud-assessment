...
SELECT * FROM licensees WHERE (eye_color= 'Hazel' AND hair_color= 'Brown') OR (eye_color='Hazel' AND hair_color='Black');

-- Question 4. Make each of Marilynn Escobar and Chris Whaley one inch shorter. Look them up by their names.

UPDATE licensees SET height=height-1 WHERE given_name='Marilynn' AND surname='Escobar';
UPDATE licensees SET height=height-1 WHERE given_name='Chris' AND surname='Whaley';
