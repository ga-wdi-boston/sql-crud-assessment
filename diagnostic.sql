...
-- given_name, gender, height, weight, born_on, eye_color, hair_color.  See
-- `data/licensees.csv` for example data.  Don't forget that all tables should
-- have an id column as a unique identifier for a row.
...
-- After completing questions 1 and 2 uncomment the following to load data from
-- `data/licensees.csv` into the `licensees` table.

\copy licensees(surname,given_name,gender,height,weight,born_on,eye_color,hair_color) FROM
'./data/licensees.csv' WITH (FORMAT csv, HEADER true)

-- Question 3. Write a query to get all attributes of licensees with Hazel eye color and Brown or Black hair color.

SELECT * FROM licensees WHERE eye_color ILIKE '%hazel%' AND WHERE hair_color ILIKE '%brown%' OR '%black%';

-- Question 4. Make each of Marilynn Escobar and Chris Whaley one inch shorter. Look them up by their names.

SELECT * FROM licensees WHERE id=4 AND id=8;

ALTER TABLE licensees WHERE id=4 AND id=8;
(surname, given_name, gender, height, weight, born_on, eye_color, hair_color) VALUES
('Escobar', 'Marilynn', 'f', '63', '143', '1944-10-09', 'Brown', 'Red');
('Whaley','Chris','o','72','177','1994-07-08','Brown','Black')

-- Question 5. Remove Dylan Rich and Teresita Myers from the table of licensees.

DELETE from licensees WHERE id=7 AND id=9;
