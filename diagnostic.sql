...
-- , given_name, gender, height, weight, born_on, eye_color, hair_color.
-- See `data/licensees.csv` for example data.  Don't forget that all tables
-- should have an id column as a unique identifier for a row.

-- create a table for clinic patients
...
INSERT INTO licensees (surname, given_name, gender, height, weight, born_on, eye_color, hair_color)
  VALUES
  ('Bishop', 'Caren', 'f', 63, 132, '1943-09-26');
);
...
SELECT eye_color, hair_color, COUNT(*)
FROM licensees
...
UPDATE licensees
SET height = height - 1
WHERE surname = 'Marilynn' AND given_name = 'Escobar'
OR surname = 'Chris' AND given_name = 'Whaley';
...
DELETE licensees
WHERE surname = 'Dylan' AND given_name = 'Rich'
OR surname = 'Teresita' AND given_name = 'Myers'
