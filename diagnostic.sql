...
--
SELECT * FROM licensees WHERE eye_color 'Hazel' AND (hair_color 'Brown' OR 'Black');

OR

SELECT * FROM licensees WHERE (hair_color 'Brown' OR 'Black') GROUP BY eye_color 'Hazel';

-- Question 4. Make each of Marilynn Escobar and Chris Whaley one inch shorter. Look them up by their names.

-- UPDATE licensees SET height = height - 1 WHERE (surname 'Whaley' AND given_name 'Chris') AND (surname 'Escobar' AND given_name 'Marilynn')

-- should I have used UNION instead of AND between the 2 licensees?....

-- Question 5. Remove Dylan Rich and Teresita Myers from the table of licensees.

--
DELETE licensees
WHERE (surname 'Rich' AND given_name 'Dylan') AND (surname 'Myers' AND given_name 'Teresita')
-- should I have used UNION instead of AND between the 2 licensees?....
