...
CREATE TABLE liscensees (
  id SERIAL PRIMARY KEY, surname TEXT, given_name TEXT, gender TEXT, height INTEGER, weight INTEGER, born_on DATE, eye_color TEXT, hair_color TEXT
);

-- Question 2. Add `Bishop, Caren, f, 63, 132, 1943-09-26, Brown, Black` into the table of licensees.

INSERT INTO liscensees(surname, given_name, gender, height, weight, born_on, eye_color, hair_color)
VALUES ('Bishop', 'Caren', 'f', 63, 132, DATE '1943-09-26', 'Brown', 'Black');
-- After completing questions 1 and 2 uncomment the following to load data from `data/licensees.csv` into the `licensees` table.

\copy liscensees(surname,given_name,gender,height,weight,born_on,eye_color,hair_color) FROM './data/licensees.csv' WITH (FORMAT csv, HEADER true)

-- Question 3. Write a query to get all attributes of licensees with Hazel eye color and Brown or Black hair color.

SELECT * FROM liscensees WHERE eye_color = 'Hazel' AND hair_color= 'Black' OR eye_color = 'Hazel' AND hair_color= 'Brown';

-- Question 4. Make each of Marilynn Escobar and Chris Whaley one inch shorter. Look them up by their names.

UPDATE liscensees
SET height = height - 1
WHERE given_name = 'Marilynn' AND surname = 'Escobar' OR given_name = 'Chris' AND surname = 'Whaley';

-- Question 5. Remove Dylan Rich and Teresita Myers from the table of licensees.

--
DELETE FROM liscensees
WHERE given_name = 'Dylan' AND surname = 'Rich' OR given_name = 'Teresita' AND surname = 'Myers'
;
