...
-- Question 1. Create a table called `licensees` to hold data in columns surname, given_name, gender, height, weight, born_on, eye_color, hair_color.  See `data/licensees.csv` for example data.
...
SELECT * FROM licensees WHERE eye_color='Hazel' AND hair_color='Black' AND hair_color='Brown';

-- Question 4. Make each of Marilynn Escobar and Chris Whaley one inch shorter. Look them up by their names.

UPDATE lincensees SET height = height - 1 WHERE surname LIKE 'Marilynn%';
UPDATE lincensees SET height = height - 1 WHERE surname LIKE 'Chris%';

-- Question 5. Remove Dylan Rich and Teresita Myers from the table of licensees.

DELETE FROM licensees WHERE surname LIKE 'Dylan%' AND given_name 'Rich%';
DELETE FROM licensees WHERE surname LIKE 'Teresita%' AND given_name 'Myers%'
