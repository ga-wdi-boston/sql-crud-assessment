...
SELECT eye_color, hair_color FROM licensees WHERE (eye_color = 'Hazel' AND eye_color = 'Brown') OR (hair_color = 'Black')

-- Question 4. Make each of Marilynn Escobar and Chris Whaley one inch shorter. Look them up by their names.

UPDATE licensees
SET weight = height - 1
WHERE (surname= 'Marilynn' AND given_name = 'Escobar') AND (surname= 'Chris' AND given_name = 'Whaley')

-- Question 5. Remove Dylan Rich and Teresita Myers from the table of licensees.

Delete licensees
WHERE WHERE (surname= 'Dylan' AND given_name = 'Rich') AND (surname= 'Teresita' AND given_name = 'Myers')
